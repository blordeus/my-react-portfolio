  ...
  return (
    <div className={`relative bg-graphite-black min-h-screen ${data.showCursor && "cursor-none"}`}>
      ...
      <div className="max-w-5xl mx-auto px-5 lg:px-0">
        ...
        {/* ── Behind the Work ───────────────────────────────────── */}
<section className="py-14 lg:py-20 border-t border-white/10">
  <div className="flex flex-col md:flex-row md:items-start gap-8 md:gap-16">
    <div className="md:w-1/2">
      <span className="block text-xs font-semibold tracking-widest uppercase text-olive-tint mb-4">
        Behind the Work
      </span>
      <h2 className="text-4xl lg:text-5xl font-serif text-cream leading-tight">
        {data.headerTaglineTwo.replace(/\s*-\s*$/, '')}
      </h2>
    </div>
    <div className="md:w-1/2 md:pt-10">
      ...
      {data.headerTaglineFour && (
        <p className="text-base text-cream/45 mt-3 leading-relaxed">
          {data.headerTaglineFour}
        </p>
      )}
    </div>
