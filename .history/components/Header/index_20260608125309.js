  ...
  return (
    <>
      <header className="flex items-center justify-between px-5 py-5 lg:px-0 lg:py-8">
        ...
        <button
          onClick={() => router.push("/")}
          className="w-10 h-10 rounded-full bg-graphite-mid border border-white/15 flex items-center justify-center focus:outline-none hover:border-white/30 transition-colors duration-200 shrink-0"
          aria-label="Home"
        >
          <span className="text-xs font-semibold text-cream tracking-wider">{initials}</span>
        </button>
